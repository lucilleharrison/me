...
def list_please() -> list:
    ...
    list_pleasea = ["why", "is", "it", "not", "sunny", "today?"]
    return list_pleasea


def int_list_please() -> list:
    """Returns a list of integers, any integers are fine."""
    int_list_pleasea = [1, 2, 3, 4, 5, 6, 7]
    return int_list_pleasea


def string_list_please() -> list:
    """Returns a list of strings, any string are fine."""
    string_list_pleasea = ["apple", "bananna", "orange", "pear"]
    return string_list_pleasea


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    d = {"name": "lucille", "age": 18, "uni": "UNSW"}
    return d
...
def is_it_5(some_number) -> bool:
    ...
    return None  # well_is_it(10)= False


def take_five(some_number) -> int:
    """Subtracts 5 from some_number."""
    take_fivea = some_number - 5
    return take_fivea
...
def make_filler_text_dictionary() -> Dict:
    ...
    for word_length in range(3, 8):
        words = []
        for _ in range(4):  # Getting 4 words for each length
            r = requests.get(url + str(word_length))
            words.append(r.text.strip())  # Remove any leading/trailing whitespace
